...
class DeserializeService {
    ...
    def deserialize(String xml)    {
        ...
        TMOrganization organization = parseOrganization((Element)rootNode.selectSingleNode(ORGANIZATION))

        TMOrganization existingOrganization = TMOrganization.findByNameAndSiteUrl(organization.name, organization.siteUrl)
     ...
     * @return
     */
    def deserialize(String xml, TMOrganization organization)    {
        Element rootNode = readXmlDocument(xml)
    ...
    def deserialize(Element rootNode, TMOrganization organization)  {

        String entityId = getAttributeByName(rootNode, ENTITY_ID)

        // get service provider, if any
        TMProvider provider = parseProvider((Element)rootNode.selectSingleNode(SP_SSO_DESCRIPTOR))
        if(provider)  {
     ...
     * @return
     */
    def logOrganization(TMOrganization org)  {
     ...
     * @return
     */
    def saveOrganization(TMOrganization org) {
        log.debug("DeserializationService.saveOrganization")
        try {
            org.save(true)
        } catch (Exception e) {
            log.error("ERROR saveOrganization")
            e.printStackTrace()
        }
        return org
    }
    ...
    def parseAttribute(Element rootNode, String attrName, String attrValue, String name)  {
        ...
        if(elem)  {
            TMAttribute attribute = new TMAttribute(name: attrValue)
    ...
    def parseAllAttributes(Element rootNode, String attrName, String name)  {
        List<TMAttribute> attributes = new ArrayList<>()
        def elems = findAllElementsByAttribute(rootNode, attrName)
        elems.forEach({e ->
            Element el = findElementByName((Element)e, name)
            attributes.add(new TMAttribute(name: ((Element)e).attributeValue(attrName), value: el.text))
        })
        return attributes
    }
    ...
    def parseEndpoints(Element rootNode, String name, String eptName)  {
        List<TMEndpoint> endpoints = new ArrayList<>()

        def elems = findAllElementsByName(rootNode, name)
        if(!elems.isEmpty())  {
            elems.forEach({ e ->
                TMEndpoint endpoint = new TMEndpoint(
                        published: false
                      , attributes: []
                      , name: eptName
                )
                ...
                if(!els.isEmpty())  {
                    TMAttribute attribute = new TMAttribute(
                            name: SERVICE_NAME
                            , value: els[0].text
                    )
     ...
     * @return
     */
    def loadProviderEndpoints(TMProvider provider, Element rootNode)  {
        List<TMEndpoint> endpoints = parseEndpoints(rootNode, SINGLE_SIGNON_SERVICE, "Single Sign On Service")
     ...
     * @return
     */
    def loadProviderAttributes(TMProvider provider, Element attributeNode)  {
        ...
        TMAttribute attribute = parseAttribute(attributeNode, NAME, OWNER_AGENCY_NAME, ATTRIBUTE_VALUE)
        if(attribute)  {
        ...
        List<TMAttribute> list = parseAllAttributes(attributeNode, NAME, ATTRIBUTE_VALUE)
        list.forEach({a ->
            if(a.name.startsWith(HTTPS) && a.value.startsWith(HTTPS))  {   // trustmarks
                TMTrustmark trustmark = new TMTrustmark(
                        status: "ACTIVE"
                        , provisional: false
                        , url: a.value
                        , name: a.name
     ...
     * @return
     */
    def loadProviderContacts(TMProvider provider, Element rootNode)  {
        TMContact techContact = parseContact(rootNode, TECHNICAL)
        if(techContact)  {
            checkForExistingContact(techContact, provider)
        }

        TMContact admContact = parseContact(rootNode, ADMINISTRATIVE)
        if(admContact)  {
            checkForExistingContact(admContact, provider)
        }

        TMContact supContact = parseContact(rootNode, SUPPORT)
        if(supContact)  {
     ...
     * @return
     */
    def checkForExistingContact(TMContact contact, TMProvider provider)  {
        TMContact existingContact = contactService.get(contact)
        if(existingContact)  {
